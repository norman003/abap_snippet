...
public section.
  ...
  methods ALV_COLOR importing !I_CAMPO type CLIKE !I_COLOR type I default 3 returning value(RT_CELL) type LVC_T_SCOL .
  methods ALV_FCATGEN exporting !ET_FCAT type LVC_T_FCAT changing !CT_TABLE type STANDARD TABLE !C_CAMPO type CLIKE optional . 
  methods ALV_FCATNAME importing !I_UNICO type CLIKE !I_LARGE type CLIKE optional changing !CS_FCAT type LVC_S_FCAT .
  methods ALV_STYLE importing !I_CAMPO type CLIKE optional !I_STYLE type LVC_STYLE default CL_GUI_ALV_GRID=>MC_STYLE_DISABLED changing !CT_CELL type LVC_T_STYL .
  methods ALVFAST_SHOW changing !CT_TABLE type STANDARD TABLE !CT_FCAT type LVC_T_FCAT .
  methods ALVGRID_SHOW .
  methods ALVLVC_EXCLUDEST changing !CT_EXCL type SLIS_T_EXTAB .
  methods ALVLVC_REFRESH changing !CS_SEL type SLIS_SELFIELD .
  methods ALVLVC_SETTOP importing !I_TYP type STRING !I_KEY type STRING !I_INFO type ANY changing !CT_HEAD type SLIS_T_LISTHEADER .
  methods ALVLVC_SHOW changing !CT_TABLE type STANDARD TABLE .
  methods ALVSALV_SHOW changing !CT_FCAT type LVC_T_FCAT !CT_TABLE type STANDARD TABLE .
  methods AMP_ME21N importing !IO_ITEM type ref to IF_PURCHASE_ORDER_ITEM_MM . 
  methods BAPI_DATAX .
  methods BAPI_SAVE importing !IT_RETURN type BAPIRETTAB !I_NUMBER type I optional exporting !E_ICON type ICON-ID !E_MESSAGE type STRING changing !CT_RETURN type BAPIRETTAB optional exceptions ERROR . 
  methods BATCH_CALL importing !I_TCODE type CLIKE !IS_PARAM type CTU_PARAMS optional !I_NUMBER type I optional exporting !E_ICON type ICON-ID !E_MESSAGE type STRING changing !CT_RETURN type BAPIRETTAB optional exceptions ERROR .
  methods BATCH_DYNPRO importing !I_PROGRAM type CLIKE !I_DYNPRO type CLIKE !I_KEY type CLIKE optional .
  methods BATCH_FIELD importing !I_FIELD type CLIKE !I_INDEX type ANY optional !I_VALUE type ANY .
  methods BATCH_RUTINA .
  methods CONST_RANGE_SET importing !IS_CONST type ANY changing !CR_RANGE type STANDARD TABLE .
  methods CONVERT_CURRENCY importing !I_DATE type DATS !I_WAERS_IN type WAERS !I_MONTO_IN type DMBTR !I_WAERS_OU type WAERS exporting !E_MONTO_OU type DMBTR .
  methods CONVERT_TO_STRING importing !IN type ANY returning value(OUT) type STRING .
  methods CURSOR_PARALLEL .
  methods CURSOR_SELECT .
  methods DATE_FROMCHAR importing !I_FECHA type CLIKE changing !E_DATUM type DATUM .
  methods DATE_INTERVAL importing !I_DATUM type SY-DATUM !I_DAY type NUMC2 optional !I_MONTH type NUMC2 optional !I_SIGN type CHAR01 default '+' !I_YEAR type NUMC2 optional returning value(E_DATUM) type SY-DATUM .
  methods DATE_STARTEND importing !I_DATE type DATUM changing !E_FECINI type DATUM optional !E_FECFIN type DATUM optional .
  methods DYNAMIC_SELECT .
  methods DYNPRO .
  methods DYNPRO_FORZAR_PBO .
  methods DYNPRO_HELP .
  methods EXCEL_UPLOAD .
  methods EXCEL_UPLOADEASY importing !I_FILENAME type CLIKE changing !RT_DATA type STANDARD TABLE .
  methods F4_FIELD .
  methods FIELD_GETDYNAMIC importing !I_FIELD type CLIKE !IS_LINE type ANY changing !E_OUT type ANY .
  methods FILE_BROWSER changing !C_DIR type CLIKE exceptions ERROR .
  methods FILE_EXIST importing !I_FILE type CLIKE changing !E_SUBRC type SY-SUBRC exceptions ERROR .
  methods FIND_REGEX .
  methods FOLDER_LISTFILES importing !I_DIR type CLIKE changing !ET_FILE type DB2_T_STRING .
  methods GOS_DOWNLOAD importing !I_FOLDER type STRING !I_OBJECT type CLIKE !I_OBJ_NAME type CLIKE exceptions ERROR .
  methods GUI_BROWSE returning value(R_FOLDER) type STRING exceptions ERROR .
  methods GUI_DOWNLOAD_BIN importing !I_FILE type STRING changing !CT_DATA type STANDARD TABLE exceptions ERROR .
  methods GUI_DOWNLOAD_TXT importing !I_FILE type CLIKE !I_SEPARATOR type XFELD !I_APPEND type XFELD optional changing !CT_DATA type STANDARD TABLE exceptions ERROR .
  methods GUI_UPLOAD_TXT importing !I_FILENAME type CLIKE !I_SEPARATOR type CLIKE changing !RT_DATA type STANDARD TABLE exceptions ERROR .
  methods HANA_RANGE .
  methods HANA_SELECT .
  methods INPUT_DATA importing !I_TITLE type CLIKE !I_TAB type CLIKE !I_FIELD type CLIKE exporting !E_VALUE type CLIKE exceptions CANCEL .
  methods IS_HANA .
  methods IS_NUMERIC .
  methods ISAUTHORITHY_VKORG importing !I_VKORG type VKORG !I_VTWEG type VTWEG !I_SPART type SPART !I_ACTVT type CLIKE exceptions ERROR .
  methods ISAUTHORITY_AUART importing !I_AUART type AUART exceptions ERROR .
  methods ISCONFIRM importing !I_QUESTION type STRING !I_CANCEL type XFELD optional exceptions CANCEL .
  methods JOB .
  methods LIST_CLASES .
  methods LIST_FUNCTIONS .
  methods LIST_PROGRAMS .
  methods MAIL_BODYFROMTLINE importing !IT_LINE type TLINE_TAB changing !CT_BODY type BCSY_TEXT .
  methods MAIL_HTMLFROMTABLE importing !I_CAMPOS type CLIKE !IT_DET type TABLE !I_COLOR type ANY optional changing !CT_BODY type BCSY_TEXT .
  methods MAIL_READCONSTEXT importing !I_FORMAT type CLIKE !I_PROGRAM type CLIKE exporting !ES_MAIL type GTTY_MAIL_DATA .
  methods MAIL_REPLACEVALUE importing !REPLACE type CLIKE !WITH type ANY changing !INTO type CLIKE .
  methods MAIL_SEND importing !IS_MAIL type GTTY_MAIL_DATA !I_TYPE type SO_OBJ_TP default 'RAW' !I_INMEDIATO type XFELD optional !I_COMMIT type XFELD optional exceptions ERROR .
  methods MEMORY importing !I_TIPO type CHAR01 !I_PROCESS type CLIKE changing !C_DATA type ANY .
  methods MEMORY_BUFFER importing !I_TIPO type CHAR01 !I_PROCESS type CLIKE changing !C_DATA type ANY .
  methods MEMORY_GET .
  methods MESSAGE_SHOW .
  methods MONTH_RANGE importing !I_DATE type DATUM returning value(R_DATE) type TRGR_DATE .
  methods READTEXT importing !I_SO10 type CLIKE optional !IS_THEAD type THEAD optional exporting !E_LINES type TLINE_TAB !E_TEXT type STRING .
  methods return_from_message changing !CT_RETURN type BAPIRETTAB .
  methods RETURN_SHOW importing !IT_RETURN type BAPIRETTAB .
  methods RETURN_SHOWMESSAGE importing !IS_RETURN type BAPIRET2 optional .
  methods SCREEN_BASIC .
  methods SCREEN_BUTTON .
  methods SCREEN_CHECKBOX .
  methods SCREEN_GETFIELD importing !I_FIELD type CLIKE !I_DYNNR type CLIKE changing !E_VALUE type ANY .
  methods SCREEN_LISTBOX .
  methods SCREEN_PF_STATUS .
  methods SCREEN_REQUIRED .
  methods SCREEN_TOOLBAR .
  methods SCREEN_UPDATE importing !I_FIELD type CLIKE !I_VALUE type CLIKE .
  methods SERVER_UPLOAD_TXT importing !I_FILENAME type STRING changing !CT_DATA type STANDARD TABLE exceptions ERROR .
  methods SMARTFORM_TO_PDF importing !IT_OTF type SSFCRESCL-OTFDATA returning value(RT_DATA) type SOLIX_TAB exceptions ERROR .
  methods SMARTFORMS importing !I_FORMNAME type TDSFNAME !I_PREVIEW type TDPREVIEW default 'X' !I_TDDEST type RSPOPNAME default 'LP01' !IT_CAB type STANDARD TABLE exceptions ERROR .
  methods SNRO_NEXT importing !IS_INRI type INRI changing !R_NUMBER type CLIKE exceptions ERROR .
  methods SUBMIT .
  methods TABLE_DOWNLOAD .
  methods TABLE_MODIFY .
  methods TCODE_CJ13 importing !I_POSID type POSID .
  methods TCODE_FB03 importing !I_BUKRS type BKPF-BUKRS !I_BELNR type BKPF-BELNR !I_GJAHR type BKPF-GJAHR .
  methods TCODE_IE03 importing !I_EQUNR type EQUNR .
  methods TCODE_MB03 importing !I_MBLNR type MBLNR !I_MJAHR type MJAHR .
  methods TCODE_ME23N importing !I_EBELN type EBELN .
  methods TCODE_ME53N importing !I_BANFN type BANFN .
  methods TCODE_MIR4 changing !CT_TABLE type STANDARD TABLE .
  methods TCODE_VA03 importing !I_VBELN type VBAK-VBELN .
  methods TLINE_SAVE importing !IS_THEAD type THEAD !I_TEXTO type CLIKE exceptions ERROR .
  methods UNIT_CONVERTMATERIAL importing !I_MATNR type MATNR !I_MEINS_IN type MEINS !I_MEINS_OU type MEINS !I_MENGE_IN type MENGE_D changing !R_MENGE_OU type MENGE_D .
  methods VALUE_DYNAMIC_GET importing !INPUT type CLIKE changing !OUT type ANY exceptions ERROR .
  methods VIEW_CALL .
...
*    ENDIF.
  ...
  METHOD batch_call. "'
    ...
    IF is_param IS INITIAL.
      ls_param-dismode = 'N'.
      ls_param-updmode = 'S'.
      "'11 ls_param-nobinpt = 'X'. "Casos especificos
    ELSE.
      ls_param = is_param.
    ENDIF.
    ...
    TRY.
        ...
        IF l_mask IS NOT INITIAL.
          ...
          CALL FUNCTION l_mask
            ...
            importing         output = <low>.
          CALL FUNCTION l_mask
            EXPORTING
              input  = ls_const-valor2
            importing         output = <high>.
        ELSE.
          <low> = ls_const-valor1.
  ...
  METHOD convert_currency. "'
    IF i_waers_in <> i_waers_ou.
      CALL FUNCTION 'CONVERT_TO_LOCAL_CURRENCY'
        ...
        importing     local_amount     = e_monto_ou
  ...
  METHOD date_startend. "'
    CALL FUNCTION 'OIL_MONTH_GET_FIRST_LAST'
      EXPORTING
        i_date      = i_date
      importing   e_first_day = e_fecini
        e_last_day  = e_fecfin.
  ENDMETHOD.
  ...
  METHOD file_browser. "'
    ...
    cl_gui_frontend_services=>directory_browse(
      changing   selected_folder      = l_dir
  ...
  METHOD gos_download. "'
    ...
    LOOP AT lt_link INTO ls_link.
      ...
      CALL FUNCTION 'SO_DOCUMENT_READ_API1'
        ...
        importing     document_data              = ls_doc
        TABLES
  ...
  METHOD gui_browse. "'
    ...
    cl_gui_frontend_services=>directory_browse(
      ...
      changing   selected_folder      = r_folder
      exceptions   cntl_error           = 1
        error_no_gui         = 2
        not_supported_by_gui = 3
    ).
...
**    filetype               = 'ASC'       "Default
  ...
  METHOD mail_htmlfromtable. "'
    ...
    "'3 Obtener campos de la tabla input
    ...
    "'4 Si contiene estructuras incluidas
          ...
          IF sy-subrc = 0.
            ...
            IF lo_elem->edit_mask IS NOT INITIAL.
              ...
              CALL FUNCTION l_function_conv
                EXPORTING
                  input  = <fs>
                importing             output = l_char.
            ELSE.
              CASE ls_comp-type->type_kind.
                  "'621.112 Montos a la derecha
                WHEN 'P'.
  ...
  METHOD mail_send. "'
    ...
    TRY.
        ...
        "'215 Adjunta titulo de correo y cuerpo
        ...
        "'217 Adjunta al objeto principal
        lo_mail->set_document( lo_doc ).


        "'218 Adjunta destinatarios
        lt_desti = is_mail-desti.
        LOOP AT lt_desti INTO ls_desti.

          "'218.1 Direccion de email
          IF ls_desti-email IS NOT INITIAL.
            ...
            "'218.11 Usuario sap obtener mail por clase
          ELSEIF ls_desti-uname IS NOT INITIAL.
            ...
            "'218.12 Usuario sap obtener mail por select
        ...
        "'2111 Forzar envio inmediato
        IF i_inmediato <> space.
          SUBMIT rsconn01 WITH mode = 'INT' AND RETURN.
        ENDIF.

        "'2112 Manejo de excepcion
...
*ENDFORM.
  ...
  METHOD screen_getfield. "'
    ...
    ELSE.
      CLEAR e_value.
    ENDIF.
    " " Screen_getfield IMPORTING 'GS_TOP101-AUDAT_L' '0101' gs_top101-audat_l.
  ENDMETHOD.
...
**    ls_option-tddest    = 'LP01'.       "Constante
**    ls_option-tdnoprev  = abap_on.
**    ls_option-tdnewid   = abap_on.
**
...
**----------------------------------------------------------------------*
  ...
  METHOD smartforms. "'
    ...
    CALL FUNCTION 'SSF_FUNCTION_MODULE_NAME'
      EXPORTING
        formname           = i_formname
      importing   fm_name            = l_fn_name
      exceptions   no_method          = 1
        no_function_module = 2
        OTHERS             = 3.
    IF sy-subrc <> 0.
      MESSAGE ID sy-msgid TYPE sy-msgty NUMBER sy-msgno
    ...
    LOOP AT it_cab ASSIGNING <fs_cab>.
      CALL FUNCTION l_fn_name
        EXPORTING
          ...
          is_cab             = <fs_cab>
**          it_det             = it_det
        exceptions     methodatting_error = 1
          internal_error     = 2
          send_error         = 3
          user_canceled      = 4
          OTHERS             = 5.
